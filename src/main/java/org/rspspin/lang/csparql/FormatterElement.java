...
public class FormatterElement extends org.apache.own.sparql.serializer.FormatterElement {
	...
	public void visitStreamBlock(Element el, boolean first) {
		if (el instanceof ElementNamedGraph) {
			if(strict)
				throw new QueryException("ERROR: CSPARQL does not support named graphs in streams.");
			else
				System.err.println("WARNING: CSPARQL does not support named graphs in streams. Triples are collapsed into the default graph.");
		}
