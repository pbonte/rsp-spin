...
public class ARQFactory {
	...
	/**
	 * Convenience method to get a named graph from the current ARQFactory's
	 * Dataset.
	 * 
	 * @param graphURI
	 *            the URI of the graph to get
	 ...
	 * local QueryExecutions generated by this factory. Does nothing by default.
	 * 
	 * @param qexec
	 *            the QueryExecution to modify
	 */
	 ...
	 * 
	 * @param spinCommand
	 *            the SPIN Command to convert to String
	 * @return the String
	 */
	...
	public String createExpressionString(RDFNode expression) {
		...
		if (result == null) {
			StringPrintContext p = new StringPrintContext();
			p.setUsePrefixes(false);
			SPINExpressions.printExpressionString(p, expression, false, false,
					expression.getModel().getGraph().getPrefixMapping());
			result = p.getString();
			if (useCaches) {
	 ...
	 * 
	 * @param model
	 *            the Model to get the prefixes from
	 * @param includeExtraPrefixes
	...
	/**
	 * Converts a SPIN Query object into a ARQ Query. This method is the
	 * recommended way for this conversion - it uses a cache to retrieve queries
	 * that it has already seen before.
	 * 
	 * @param spinQuery
	 *            the SPIN query
	...
	/**
	 * Creates the "physical" Jena Query instance. Can be overloaded to create
	 * engine-specific Query objects such as those for AllegroGraph.
	 * 
	 * @param queryString
	 *            the parsable query string
	 * @param prefixMapping
	 *            an optional PrefixMapping to initialize the Query with (this
	...
	/**
	 * Creates a new Query from a partial query (possibly lacking PREFIX
	 * declarations), using the ARQ syntax specified by <code>getSyntax</code>.
	 * 
	 * @param model
	...
	/**
	 * Creates a QueryExecution for a given Query in a given Model, with no
	 * initial bindings. The implementation basically uses Jena's
	 * QueryExecutionFactory but with the option to use different Dataset as
	 * specified by <code>getDataset(model)</code>.
	 * 
	 * @param query
	 *            the Query
	 * @param model
	...
	/**
	 * Creates a QueryExecution for a given Query in a given Model, with some
	 * given initial bindings. The implementation basically uses Jena's
	 * QueryExecutionFactory but with the option to use different Dataset as
	 * specified by <code>getDataset(model)</code>.
	 * 
	 * @param query
	 ...
	 *            the initial variable bindings or null
	 * @return a QueryExecution
	 */
	...
	/**
	 * Specifies a Dataset that shall be used for query execution. Returns a new
	 * DatasetImpl by default but may be overloaded in subclasses. For example,
	 * TopBraid delegates this to the currently open Graphs.
	 * 
	 * @param defaultModel
	 *            the default Model of the Dataset
