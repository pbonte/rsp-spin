...
public class FormatterElement extends FormatterBase implements ElementVisitor {
	...
	// /** Control whether triple patterns always have a final dot - it can be
	...
	/**
	 * Control whether a group of one is unnested - changes the query syntax
	 * tree
	 */
	...
	public void visit(ElementGroup el) {
		if (GROUP_UNNEST_ONE && el.getElements().size() == 1) {
			// If this is an element of just one, we can remove the {} if it is
			// a group.
	...
	protected void formatTriples(BasicPattern triples) {
		...
		// Loops:
		List<Triple> subjAcc = new ArrayList<Triple>(); // Accumulate all
														// triples with the same
														// subject.
