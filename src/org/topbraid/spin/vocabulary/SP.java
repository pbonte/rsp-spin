...
public class SP {
	...
	private static void init(Personality<RDFNode> p) {
		...
		p.add(org.topbraid.spin.model.update.Delete.class,
				new SimpleImplementation(Delete.asNode(), org.topbraid.spin.model.update.impl.DeleteImpl.class));
		...
		p.add(org.topbraid.spin.model.update.Insert.class,
				new SimpleImplementation(Insert.asNode(), org.topbraid.spin.model.update.impl.InsertImpl.class));
	...
	/**
	 * Checks whether the SP ontology is used in a given Model. This is true if
	 * the model defines the SP namespace prefix and also has sp:Query defined
	 * with an rdf:type. The goal of this call is to be very fast when SP is not
	 * imported, i.e. it checks the namespace first and can then omit the type
	 * query.
	 * 
	 * @param model
	 *            the Model to check
	...
	public static boolean exists(Model model) {
		return model != null && SP.NS.equals(model.getNsPrefixURI(SP.PREFIX))
